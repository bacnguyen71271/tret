...
const apikey = "BTJWZ9FUNUBA6RE21W5RUN5BZ4XGNPWNEP";
                ...
                if (err){
                    ...
                    if(result['status'] == 1){
                        ...
                        for(var i =0; i < blocklength;i++){
                            ...
                            let query_checkWallet = "SELECT * FROM `wallet` WHERE `address` = '"+wallet+"' LIMIT 1";
                            ...
                            db.query(query_checkWallet,(err,result)=>{
                                if(err){ console.log("Wallet error 4: "+err.message)}else{
                                    ...
                                    if(result.length > 0){
                                        ...
                                        let checkTrans = "SELECT * FROM `transaction` WHERE `block` = '"+block+"' LIMIT 1";
                                        ...
                                        db.query(checkTrans,(err,result)=>{
                                            if(err){ console.log("Wallet error 5: "+ err.message)}else{
                                                ...
                                                if(result.length <= 0){
                                                    let updatewallet = "UPDATE `wallet` SET `avalible_token`= '"+ tong +"' WHERE `address` = '"+wallet+"'";
                                                    let insertTrans = "INSERT INTO `transaction`(`block`, `id_wallet`, `tx`, `type_trans`,`status`, `value`, `token_name`, `timeStamp`) VALUES ('"+block+"','"+idWallet+"','"+transactionHash+"','in','success','"+data+"','TRET','"+timeStamp+"')";
                                                    db.query(updatewallet,(err,result)=>{});
                                                    db.query(insertTrans,(err,result)=>{
                                                        if(!err){
                                                            let insertNotifi = "INSERT INTO `notification`(`id_user`, `avatar`, `link`, `content`, `timer`, `status`) VALUES ('"+userid+"','TRET','wallet_"+result['insertId']+"','IN_TRET_"+data+"','"+Date.now()+"','0')"
                        ...
                        //Update Block
                        if(Blocknumber !== 0){
                            let query = "UPDATE `options` SET `	last_block_token_check`='"+Blocknumber+"' WHERE `option_id` = '1'"
                            db.query(query);
                        }
                    }else{
...
setInterval(()=>{
    ...
    //Lấy địa chỉ ví
    let queryGetWallet = "SELECT * FROM `wallet` WHERE 1";
                                ...
                                if(a > 0){
                                    for(var j = 0; j < a; j++){
                                        ...
                                        if( input == '0x' && iserror == 0 && to == walet){
                                            let checkTrans = "SELECT * FROM `transaction` WHERE `block` = '"+blnb+"' LIMIT 1";
                                            db.query(checkTrans,(err,result)=>{
                                                if(err){ console.log("Balance ETH Error 3: "+ err.message);}else{
                                                    //Ghi nhận giao dịch đến ví
                                                    if(result.length <= 0){
                                                        let query_checkWallet = "SELECT * FROM `wallet` WHERE `address` = '"+walet+"' LIMIT 1";
                                                        ...
                                                        db.query(query_checkWallet,(err,result2)=>{
                                                            ...
                                                            if(err){ console.log("Balance ETH Error 5: "+err.message)}else{
                                                                let updatewallet = "UPDATE `wallet` SET `eth_balance`= '"+ totalEth +"',`blockNumber`='"+blnb+"' WHERE `address` = '"+walet+"'";
                                                                let insertTrans = "INSERT INTO `transaction`(`block`, `id_wallet`, `tx`, `type_trans`,`status`, `value`, `token_name`, `timeStamp`) VALUES ('"+blnb+"','"+walet_id+"','"+transactionHash+"','in','success','"+value+"','ETH','"+timeStamp+"')";
                                                                
                                                                db.query(updatewallet,(err,result)=>{
                                                                    if(!err){
                                                                        db.query(insertTrans,(err,result)=>{
                                                                            if(!err){
                                                                                let insertNotifi = "INSERT INTO `notification`(`id_user`, `avatar`, `link`, `content`, `timer`, `status`) VALUES ('"+UserID+"','TRET','wallet_"+result['insertId']+"','IN_ETH_"+value+"','"+Date.now()+"','0')"
                                                                                db.query(insertNotifi,(err,result)=>{});
                                                                            }
                                                                        });
                                                                    }
