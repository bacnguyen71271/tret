...
module.exports = {
    ...
    settings_changepass_post:(req,res)=>{
        if(req.body.old_password == undefined || req.body.new_password == undefined || req.body.retype_password == undefined){
            ...
            //Check Password
            let checkpass = "SELECT * FROM `user` WHERE `userid`= '"+req.session.username+"' AND `password`='"+md5(req.body.old_password)+"' LIMIT 1"
                ...
                if(!err){
                    if(ressu.length > 0 ){
                        if(req.body.new_password !== req.body.retype_password){
                            ...
                            let change = "UPDATE `user` SET `password`= '"+md5(req.body.new_password)+"',`updated_at`= '"+Date.now()+"' WHERE `userid` = '"+req.session.username+"'";
