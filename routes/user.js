...
module.exports = {
    auth: (req, res) => {
        if (req.params.token == undefined) {
            ...
            jwt.verify(req.params.token, 'tretsecretemail', function (err, decoded) {
                if (err) {
                    ...
                    //Nếu không có lỗi gì
                    let query = "SELECT * from `user` where `email`='" + decoded['e'] + "'";
                    db.query(query, (err, result) => {
                        ...
                        if (err) {
                            ...
                            if (result[0]['status'] !== 0) {
                                ...
                                let query = "UPDATE `user` SET `status`= '1',`updated_at`= '" + Date.now() + "' WHERE `email`='" + decoded['e'] + "'";
                                db.query(query, (err, result) => {
                                    if (err) { }
                                });
                                //Tạo thư mục media
    ...
    loginPage: (req, res) => {
        ...
        let query = "SELECT * FROM `user` WHERE `email` = '" + req.body.email + "' or `username` = '" + req.body.email + "' LIMIT 1";
        db.query(query, (err, result) => {
            if (err) {
            } else {
                if (result.length == 0) {
        ...
        if(req.params.token == undefined){
            ...
            jwt.verify(req.params.token, 'token', function (err, decoded) {
                if (err) {
                    ...
                    let query = "SELECT * FROM `user` WHERE `email` = '" + decoded + "' or `username` = '" + decoded + "' LIMIT 1";
                    db.query(query, (err, result) => {
                        if(!err){
